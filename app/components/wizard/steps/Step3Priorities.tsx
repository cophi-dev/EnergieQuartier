...
export function Step3Priorities({
  ...
  return (
    <div className="space-y-6">
              ...
              render={({ field }) => (
                <div className="space-y-3">
                  <Slider
                    ...
                    className="[&_[data-slot=slider-range]]:bg-[#088395]"
                  />
                  <div className="flex justify-between text-sm">
                    <span className="text-[#0A4D68]/50">Niedrig</span>
                    <span className="font-semibold text-[#0A4D68] dark:text-[#00FFCA]">
                      {field.value} %
                    </span>
                    <span className="text-[#0A4D68]/50">Hoch</span>
                  </div>
                </div>
              )}
            />
      ...
      <div className="rounded-xl border border-[#0A4D68]/10 bg-[#F5F8FA] p-4 dark:bg-[#0A4D68]/40">
        <p className="text-xs font-medium text-[#088395] mb-3">
          Normalisierte Gewichtung (Vorschau)
        </p>
        <div className="flex h-3 overflow-hidden rounded-full">
          <div
            className="bg-[#0A4D68] transition-all"
            style={{ width: `${normalizedPreview.cost}%` }}
            title={`Kosten ${normalizedPreview.cost}%`}
          />
          <div
            className="bg-[#088395] transition-all"
            style={{ width: `${normalizedPreview.co2}%` }}
            title={`CO₂ ${normalizedPreview.co2}%`}
          />
          <div
            className="bg-[#00FFCA] transition-all"
            style={{ width: `${normalizedPreview.autarky}%` }}
            title={`Autarkie ${normalizedPreview.autarky}%`}
          />
        </div>
        <div className="mt-2 flex justify-between text-xs text-[#0A4D68]/70 dark:text-white/70">
